...
| 通知方法 | メリット | デメリット |
| --------------- | --------------- | --------------- |
| 1. EXTERNAL ACCESS INTEGRATION利用法   | snowflake内で完結できて、通知スタイルのカスタマイズも可能 | ネットワークのルールについて考える必要がある |
| 2. slackメアド通知法   | 単純かつ素早く実装できる    | emailの通知なのでスタイルのカスタマイズが難しい   |
| 3. クラウドメッセージング法   | クラウドサービス利用による堅牢なシステム構成が構築可能 | 各種クラウドアカウントが必要で保守対象が増える  |

社内のネットワークポリシーが特に厳格でなければ1, とにかくいちばん簡単な方法で通知さえできればいいんだというケースは2, AWSなどのクラウドシステムがすでにあり保守もできるという話なら3、になるかなといった所管です。個人的にはsnowflake内で完結できて通知スタイルのカスタマイズも可能な **`1. EXTERNAL ACCESS INTEGRATION利用法`** がおすすめで、弊社でも取り入れています。
